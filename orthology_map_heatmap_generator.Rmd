...
## CHANGE ##
```{r include=TRUE}
# Not: These files correspond to gene lists generated from different logFC values.
...
"""
```{r include=TRUE}
corR.output <- corr.test(x= ciona_gsm_v4,
                         y = zeb_gsm_v4,
                         method = "spearman",
                         adjust = "fdr")
```
...
```{r include=TRUE}
# Corr legend range. 
min.index <- min(corR.mat)
max.index <- max(corR.mat)
leeway <- 0.01
limits <- c(min.index-leeway, max.index+leeway)

# Aesthetics and labels. 
your.title <- "Larva CTs vs. Zeb CTs, logFC > 0"
...
"""
```{r include=TRUE}
num.sig <- sum(corR.pval.mat < 0.05)
num.sig
```
